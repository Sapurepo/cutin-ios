...
### 릴리즈 0.2.0 범위 (완료 · v0.2.0) — 백엔드 실계약
...
### 릴리즈 0.3.0 범위 (진행 중) — 지키지 못한 약속을 지킨다

0.2.0은 계약을 전부 붙이고도 **지키지 못한 약속**을 셋 남겼다: 알림이 실제로 오지 않고,
Release 구성이 첫 호출에서 멈추고(서버 미배포), 대표 컷은 계약만 있다. 0.3.0은 새 기능이
아니라 그 간극을 닫는 릴리즈다.

#### 들어간 것

- **알림 · 디바이스** (#28) — notifications 3 + devices 2 + notification-preferences 2,
  일곱 오퍼레이션. 인앱 알림 목록(피드 툴바 종 + 미읽음 점) · 읽음 처리(본 것만, 100개 분할) ·
  설정(슬롯·끄기) · APNs 토큰 등록 경로. **발송은 아직 안 된다** — 아래 참고.
- **대표 컷 §6.3** (#31) — 마무리 단계에서 지정, 프로필 그리드가 합성본 대신 대표 컷을
  보여주고 직접 지정한 포스트는 그리드 맨 앞에 고정된다(핀 배지). 남은 둘은 서버 지원이
  선행이라 [cutin-backend#13](https://github.com/Sapurepo/cutin-backend/issues/13)으로
  요청해 뒀다: **발행 후 고정 해제/지정**(지금은 `POST_NOT_DRAFT`가 모든 PATCH를 거절 —
  버튼만 만들면 항상 실패하는 거짓 약속이라 서버가 열리면 붙인다)과 **고정 우선 정렬**
  (지금은 받아 온 페이지 안에서만 참). "직접 지정"의 판정은 nil이 아니라 **0이 아닌 값**이다 —
  서버가 발행 시 미지정을 0으로 채워 발행된 포스트는 전부 non-null이기 때문(0.4.0 감사 B3).
- **서비스 팁 §3.5** (#32) — 캐러셀 3장 + 시작하기. 온보딩 직후 한 번, 프로필 도움말에서
  재열람. 세 장 모두 실제로 있는 기능만 말한다 — 미뤄 온 이유("지어내면 거짓말")가 해소됐다.

#### 남은 축과 막힌 곳

푸시가 실제로 오려면 **세 조각**이 맞아야 한다. 서버 쪽 발송 스케줄러는 이미 있다
(`reminderJob` — pg-boss cron, 15분 tick, 디바이스 타임존 기준 슬롯 판정).
...
- **§3.2 연락처 매칭** — 서버에 경로가 없고 백엔드 로드맵에도 없다.
- **오프라인 피드 · 구글 로그인** — 0.2.0과 같은 이유.
...
Cutin/
  ...
  Resources/Sounds/     효과음 넷(tap · tick · pop · success) — Scripts/sounds/make.py가 만든다
Config/
...
```
Design/                 0.4.0 UI 개편의 판단 기록 — 브리프 · 계획 · 감사(콘택트 시트) · 결정 기록
```
