#!/usr/bin/env python3
"""효과음 셋을 합성해 `Cutin/Resources/Sounds/*.caf`로 쓴다.

의존성 없음(표준 라이브러리 + macOS `afconvert`). 샘플을 다운로드하지 않고 만드는 이유는
라이선스 문구가 필요 없고, 이 파일을 고쳐 다시 돌리면 같은 소리가 나오기 때문이다.

  tick     카운트다운 3·2·1 — 짧은 톡. 셔터에는 안 얹는다(시스템 셔터음이 이미 난다).
  pop      반응을 남길 때 — 아래로 미끄러지는 짧은 팝.
  success  발행 완료 — 두 음(G5→D6)이 잠깐 겹치며 사라진다. Haptics.success와 함께.

시스템 사운드 API(`AudioServicesPlaySystemSound`)로 재생하므로 30초 이내 · 선형 PCM이면 된다.
16-bit 모노 44.1kHz. 피크는 -9 dBFS 안팎 — 벨소리 볼륨을 따르니 키보드 톡보다 크지 않게.
...
def pop():
    ...
    for i in range(n):
        t = i / RATE
        f = 750 * (350 / 750) ** (t / dur)   # 750→350Hz 지수 글라이드
...
if __name__ == "__main__":
    ...
    write("tick", tick(), 0.35)
    write("pop", pop(), 0.35)
    write("success", success(), 0.4)
