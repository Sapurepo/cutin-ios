...
| ⑥ | 비우는 순간 날아가 있던 요청이 돌아와도 옛 목록을 되살리지 않는다 |
...
⑥의 대조군은 `load`의 세대 검사를 빼는 것입니다.

```swift
guard generation == self.generation else { return }   // 이 줄들을 지운다
```

```
FAIL  피드가 비어 있다  — 2건
FAIL  hasLoaded가 되살아나지 않았다
FAIL  친구 목록이 비어 있다  — 1건
=== 결과: 실패 5건 ===
```
...
## 스텁을 만들며 밟은 것 (둘 다 "통과가 거짓일 수 있다"는 예다)

**관계 목록 경로를 늦게 넣었습니다.** `/users/me/friends`가 `/users/{id}` 프로필 분기로 흘러
`UserPage` 디코드가 실패했고, ⑥의 SocialStore 검사가 **경로를 밟지도 않은 채 통과**했습니다.
음성 대조군을 돌렸을 때 PostStore 쪽만 실패하고 SocialStore 쪽은 통과하는 것을 보고 알았습니다 —
대조군이 없었으면 그대로 넘어갔습니다.
