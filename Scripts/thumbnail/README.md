...
| | 보는 것 |
|---|---|
| ① | 지정 인덱스를 `cutIndex`로 찾음(역순 배열) · 미지정 → 첫 컷 · 없는 인덱스 → 가장 앞 컷 · 컷 없음 → 합성본 → nil · `isPinned` 판정(**0은 서버 기본이라 고정이 아니다** · 서버가 `pinned`를 주면 그 값이 우선 — cutin-backend#14) |
...
`isPinned`의 음성 대조군은 nil 검사로 되돌리는 것입니다 — 0.3.0이 그렇게 짜여 있었고,
서버가 발행 시 미지정을 0으로 채우므로 **발행된 포스트 전부에 핀이 붙었습니다**(2026-08-17 감사 B3.
0.3.0 하니스는 표본이 "지정 = 0"이라 못 잡았습니다).
...
```
FAIL  0은 서버가 채우는 기본 — 고정이 아니다
FAIL  고정 앞으로 + 그룹 안 발행 순서 보존  — ["A", "B", "C", "E", "D"]
=== 결과: 실패 2건 ===
```

## 결과 (2026-08-17 · iPhone 17 Pro · iOS 26.5)

- 정상: **14 PASS / 0 FAIL** (0.4.0에서 `pinned` 우선 2건 추가)
- 음성 대조군(배열 위치 인덱싱): **실패 2건** (2026-08-14 기록)
- 음성 대조군(nil 검사): **실패 2건**
