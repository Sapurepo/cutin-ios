 ...
 *
 * `RemoteImage`(AsyncImage)를 쓴다. `/media/content/…`는 인증을 요구하지 않고(키에 소유자·미디어 id가 들어가
    ...
    var body: some View {
        ZStack {
            ...
            if let url, let parsed = URL(string: url) {
                 ...
                 * 화면을 열 때마다 빈 원이 한 번 번쩍인다. 그래서 자리 색도 반짝임도 없다. */
                RemoteImage(url: parsed, placeholder: .clear, shimmers: false)
            }
        }
